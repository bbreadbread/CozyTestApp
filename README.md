...
## Описание
**CozyTest** — это приложение для автоматизации учебного процесса, связанного с тестированием участников конкурса «Безопасное колесо». Приложение разработано на **WPF** и для своей работы использует **.NET 8.0** и **MS SQL Server**.
...
```
git clone https://github.com/bbreadbread/CozyTestApp_InstallationFiles
```

**ИЛИ:**
1. Перейдите по ссылке и скачайте архив:
   https://github.com/bbreadbread/CozyTestApp_InstallationFiles
2. Распакуйте ZIP-архив в **пустую папку**.

### Состав установленных файлов
- **Curator/** — приложение для регистрации преподавателей
  > ⚠️ Рекомендуется удалить эту папку на ученических ПК после установки
- **Main/** — основное приложение и изображения для тестов
- **struct.sql** — SQL‑скрипт создания структуры БД
...
4. Выполните по очереди SQL‑скрипты:
   - `struct.sql` — создание структуры БД
...
3. Замените параметр сервера на наименование вашего сервера:
   ...
   ```
4. Сохраните файл
5. После успешного выполнения запустите файл `CozyTest.exe`

---

## Создание нового пользователя (преподавателя)

1. Перейдите в папку **Curator**
2. Откройте файл `connection.settings`
3. Укажите имя сервера:
   ...
   ```
4. Сохраните файл
5. Запустите `RegistrationCuratorCozyTest.exe`
6. Зарегистрируйте нового преподавателя

### Создание тестируемых
- Войдите в основное приложение под аккаунтом преподавателя
- Перейдите в меню **«Тестируемые»**
- Создайте тестируемых, выбирайте привязку к нужному аккаунту
...
1. Создайте нового пользователя (Login) для БД в **MSSMS** и назначьте ему доступ к базе `cozy-test`. Пример:
  <table align="center">
  <tr>
    <td align="center">
...
3. Измените строку подключения:
   ```
   (Server=ИМЯ_СЕРВЕРА,ПОРТ;Database=cozy-test;User Id=ИМЯ_ПОЛЬЗОВАТЕЛЯ;Password=ПАРОЛЬ;TrustServerCertificate=True)
   ```
4. Сохраните файл
5. Запустите `CozyTest.exe`

Для удобства рекомендуется создать ярлык на рабочем столе:
**ПКМ по CozyTest.exe → Отправить → Рабочий стол**

---

## Примечания
- Папку **Curator** необходимо удалить с ученических ПК во избежание возможности создания тестируемыми новых аккаунтов преподавателей.
